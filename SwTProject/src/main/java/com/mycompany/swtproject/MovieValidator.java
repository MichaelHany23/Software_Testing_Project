...
public class MovieValidator {
    ...
    private void MovieValidate() {
        ...
        for (int i = 0; i < movies.size(); i++) {
            ...
            // id must contain all the capital letters in the title followed by 3 unique
            // numbers
            // validate id of this movie
            String id = movies.get(i).getMovieId();

            if (id.length() != (titleparts.length - NonAlphabet_count + 3)) // if more than the expected length break ,
                                                                            // error
            {
